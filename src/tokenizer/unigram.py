...
class Unigram(Tokenizer):
  def __init__(self, vocab_size: int= 1000, max_sub_len: int= 8, em_iters: int= 3, n_per: float= 0.8, special_token: Optional[Union[List, Dict]]= None, epsilon: float= 1e-10, alpha: float= 1e-7, unk_penalty: float= 1e-8):
    ...
    self.alpha = alpha  # Add-a smoothing
    self.unk_penalty = unk_penalty  # OOV penalty
  ...
  def train(self, corpus: List[str]) -> Dict[str, int]:
    ...
    while len(self.vocab) >= self.target_size - len(self.special_token):
      ...
      self._prune_vocab(losses); pbar.update(1)
  ...
  def _init_seed_vocab(self, corpus: List[str]):
    ...
    self.probs = {token: 1.0 / len(self.vocab) for token in self.vocab}   # 초기 확률 균등분포
  ...
  def _run_em(self, corpus: List[str]):
    ...
    for _ in range(self.em_iters):
      ...
      self.probs = {token: (self.counts.get(token, 0) + self.alpha / total) for token in self.vocab}
      self._normalize_probs()
  ...
  def _compute_losses(self):
    ...
    for token in self.vocab:
      if token in self.counts:
        losses[token] = self.counts[token] * safe_log_prob(self.probs[token], self.epsilon) # frequency * log probability
      else:
    ...
    for i in range(n + 1):
      for j in range(max(0, i-self.max_sub_len), i):
        ...
        if token in self.probs:
          prob = safe_log_prob(self.probs.get(token, self.unk_penalty), self.epsilon)
